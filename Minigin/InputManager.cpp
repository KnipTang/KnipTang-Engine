...
dae::InputManager::InputManager() :
	m_pimpleControlClass{ new ControllerPimpl{} }
{
}

dae::InputManager::~InputManager()
{
	delete m_pimpleControlClass;
	m_pimpleControlClass = nullptr;
}

bool dae::InputManager::ProcessInput(float deltaTime)
{
	return m_pimpleControlClass->ProcessInput(deltaTime);
}

void dae::InputManager::BindCommand(WORD button, InputActionType inputAction, std::unique_ptr<Command> command)
{
	m_pimpleControlClass->BindCommand(WORD(button), inputAction, std::move(command));
}

void dae::InputManager::BindCommand(int key, InputActionType inputAction, std::unique_ptr<Command> command)
{
	m_pimpleControlClass->BindCommand(key, inputAction, std::move(command));
}